...
| `code` | Cuándo aparece |
|---|---|
| `MALFORMED_JSON` | El body del request no es JSON válido. El `field` es `_root`. |
...
| `SCHEMA_MISMATCH` | El body no satisface una restricción `oneOf` / `anyOf` (ej. debes proveer *uno de* varias alternativas). |
