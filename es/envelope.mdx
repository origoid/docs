...
| Campo | Tipo | Descripción |
|---|---|---|
| `field` | string | Path en notación de punto al campo problemático (ej. `curp`, `address.street`). `_root` cuando el issue aplica al body completo. |
| `code` | string | Código estable de máquina. Branchea sobre este, nunca sobre `message`. Lista completa en [Errores → Códigos de validación](/es/errors#codigos-de-validacion). |
