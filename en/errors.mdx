...
|---|---|
| `MALFORMED_JSON` | The request body is not valid JSON. The `field` is `_root`. |
...
| `SCHEMA_MISMATCH` | The body does not satisfy a `oneOf` / `anyOf` constraint (e.g. you must provide *one of* several alternatives). |
...
- `INVALID_RFC_FORMAT` (`/mex/sat/v1/rfc-validations`, SAT 69 / 69-B searches)
